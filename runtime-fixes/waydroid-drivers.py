...
# setupBinderNodes keeps the last existing candidate.  Put the dedicated
# Waydroid nodes last so they win when Halium's standard nodes also exist.
BINDER_DRIVERS = [
    "binder",
    "bonder",
    "puddlejumper",
...
def setupBinderNodes(args):
    ...
    if args.vendor_type == "MAINLINE":
        ...
        # Halium kernels may expose the standard statically allocated binder
        # nodes without binderfs. They are valid for Waydroid even though the
        # image vendor type is not MAINLINE.
        for node in BINDER_DRIVERS:
            if os.path.exists("/dev/" + node):
                has_binder = True
                args.BINDER_DRIVER = node
